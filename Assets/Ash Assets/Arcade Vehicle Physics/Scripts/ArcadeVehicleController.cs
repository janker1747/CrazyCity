...
namespace ArcadeVP
{
    public class ArcadeVehicleController : MonoBehaviour
    {
        ...
        private float radius, horizontalInput, verticalInput, jumpInput;
        ...
        private void Update()
        {
            if (overrideInput)
            {
                horizontalInput = overrideHorizontal; // turning input from AI / external controller
                verticalInput = overrideVertical; // acceleration input from AI / external controller
...
                float sign = Mathf.Sign(carVelocity.z);
                ...
                if (kartLike && jumpInput > 0.1f) { TurnMultiplyer *= driftMultiplier; } //turn more if drifting
...
                if (movementMode == MovementMode.AngularVelocity)
                {
                    if (Mathf.Abs(verticalInput) > 0.1f && jumpInput < 0.1f && !kartLike)
                    {
